...
def is_potential_phishing(url):
    ...
    # Ponderar la longitud del dominio
    if domain_length <= 10:
        suspicion_score += 10
    elif domain_length <= 15:
        suspicion_score += 5
    
    # Ponderar la coincidencia con palabras clave sospechosas
    if contains_suspicious_keywords:
        suspicion_score += 30
    
    # Ponderar la coincidencia con estándares ISO
...
print(f"La probabilidad de que el URL proporcionado sea phishing es del {phishing_likelihood:.2f}%.")
